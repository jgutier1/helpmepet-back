...
public class DelitoController {
    ...
    @ApiOperation(value = "Obtiene todos delitos con detalles del usuario",
            responseContainer = "List",
            produces = "application/json",
            httpMethod = "GET")
    @GetMapping("/all")
    @Secured("ROLE_ADMIN")
    @ResponseStatus(code = HttpStatus.OK)
    public ResponseEntity<List<DelitoDTO>> indexWithUser() {
        return ResponseEntity
                .ok()
                .body(
                        delitoService.consultarTodosDetallado()
                );
    }
    ...
    @ApiOperation(value = "Eliminar un Delito",
            response = DelitoDTO.class,
            produces = "application/json",
            httpMethod = "DELETE")
    @Secured("ROLE_ADMIN")
